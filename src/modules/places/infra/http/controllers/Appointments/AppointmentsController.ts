...
import UpdatePaidAppointmentService from '@modules/places/services/UpdatePaidAppointmentService';
...
export default class AppointmentsController {
  public async consultarCobrancaAilos(
    ...
    const servico = container.resolve(ConsultarCobranca);

    const { id } = request.query;
    const ok = await servico.execute(String(id));
    return response.json(ok);
  }

  public async AilosWebHook(
    request: Request,
    response: Response,
  ): Promise<Response> {
    const filePath = path.join(__dirname, 'request-body.txt');

    // Converte o request.body para uma string JSON
  ...
  public async payPix(request: Request, response: Response): Promise<Response> {
    const servico = container.resolve(ExecutePayment);
    const ok = await servico.execute(request.body);
    return response.json(ok);
  }
  ...
  public async pixPaid(
    request: Request,
    response: Response,
  ): Promise<Response> {
    const updatePaidAppointmentService = container.resolve(
      UpdatePaidAppointmentService,
    );

    const { event } = request.body;

    if (String(event.type) === 'bill_paid') {
      const appointment = await updatePaidAppointmentService.execute(
        String(event.data.bill.charges[0].id),
      );
